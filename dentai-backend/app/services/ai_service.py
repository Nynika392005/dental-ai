...
async def analyze_image_task(image_base64: str, task_type: str) -> dict:
    ...
    try:
        prompts = {
            ...
            "habit": (
                "Analyze this image for signs of oral habits. "
                "Respond ONLY with this JSON structure: "
            ...
            "medicine": (
                "You are a pharmaceutical expert. Look at this medication image carefully. "
                "CRITICAL RULES:\n"
                "1. ONLY identify the medication if you can clearly see text, brand names, imprints, or distinctive packaging\n"
                "2. If the pill/tablet has NO visible text, markings, or identifiable features, you MUST respond with name='Cannot identify - no visible markings'\n"
                "3. DO NOT guess or make assumptions based only on color or shape\n"
                "4. If you cannot confidently identify the specific medication, return a warning message\n\n"
                "Examine: pills, tablets, bottles, packaging, text, imprints, colors, shapes, and any visible markings.\n\n"
                "Respond ONLY with this JSON structure:\n"
                "{\"name\": \"<specific medicine name with brand and strength, OR 'Cannot identify - no visible markings' if unclear>\", "
                "\"medical_purpose\": \"<what condition it treats, OR warning message if unidentified>\", "
                "\"dosage_instructions\": \"<standard dosage, OR 'Consult healthcare provider' if unidentified>\", "
                "\"safety_warnings\": \"<important warnings, OR 'Do not take unidentified medication without prescription' if unclear>\"}"
            ),
        }
